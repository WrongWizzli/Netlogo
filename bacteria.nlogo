...
to setup-map
  ...
  if map-type = "blackhole" [
    ask patches [
      if pxcor >= -10 and pxcor <= 10 and pycor >= -10 and pycor <= 10 [ set free? false]
    ]
  ]
  if map-type = "bricks" [
    ask patches [
...
to setup-colony
  ask bacterias [ die ]
  let rand-patch one-of free-patches
  let px [pxcor] of rand-patch
  let py [pycor] of rand-patch
  create-bacterias colony-size [
    setxy px py
    set size 0.8
    set shape "circle"
    set color rgb 0 random 255 random 255
    set val eval-me
  ]
  reset-ticks
end
...
to move
  ...
  let need-rotate? (p = nobody or not [free?] of p)
  if need-rotate? [ rt random 360 stop ]
  if not need-rotate? [ fd vel ]
  process-hemotacsis
end
